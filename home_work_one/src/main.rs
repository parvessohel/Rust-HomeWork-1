...
fn main() {
    ...
    get_sh1kari(x);
    let x = x + 3;
    get_sh1kari(x); // -> Here this x is the second one. As soon as a new variable is declared with the same name the earlier one is dropped and cannot be accessed. This is called Shadowing in Rust
